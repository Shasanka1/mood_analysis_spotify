...
def main():
    ...
    token = history.get_token(username, client_id, 
                              client_secret, redirect_uri, scope) #get token
    
    streamings = history.get_streamings() #get streamings history
    print(f'Recovered {len(streamings)} streamings.') #print number of streamings
    ...
    if len (tracks_without_features):
        ...
        features_dataframe['track Number'] = range(1, len(features_dataframe) + 1)
        print( f'Total number of tracks: {len(features_dataframe)}')
    ...
    for streaming in sorted(streamings, key= lambda x: x['endTime']):
        ...
        if features:
            ...
            # print(f'Streaming with features : {len(streamings_with_features)}')
